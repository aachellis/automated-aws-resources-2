...
try:
    bucket_name = ssm.get_parameter(Name=f"/{conf['name'].replace(' ', '-')}/bucket-name")['Parameter']['Value']
except botocore.exceptions.ClientError as error:
    if error.response["Error"]["Code"] == "ParameterNotFound":
        bucket_name = conf["name"].replace(' ', '-') + "-" + str(uuid.uuid4()) + "-" + conf["region"]
        ssm.put_parameter(Name=f"/{conf['name'].replace(' ', '-')}/bucket-name", Value=bucket_name, Type='String')
    else:
        raise error
